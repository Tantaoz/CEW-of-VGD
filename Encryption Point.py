...
def Henon(XList, YList, x0, y0, a, b, t0):
    N = len(XList)
    xi = x0
    yi = y0
    Lx, Ly, RxLi, RyLi = [], [], [], []
    ...
    for i in range(0, N):
        XList[i], XList[RxLi[i]] = XList[RxLi[i]], XList[i]
        YList[i], YList[RyLi[i]] = YList[RyLi[i]], YList[i]
    return XList, YList
...
def Dn_point(X_sumen, Y_sumen, XLst, YLst):
    XLst1, YLst1 = XLst, YLst
    t = 0
    for i in range(feature_num):
        for j in range(len(XLst[i])):
